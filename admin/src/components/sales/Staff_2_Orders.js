...
const Staff2Router = ({
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:10000/staff2/orders`, {
        params: {
          page,
          status: filterStatus,
  ...
  const handleCreateShipment = (order) => {
    navigate("/create-shipmentS", { state: { order } });
  };
                  ...
                  <>
                    <Form.Group>
                      ...
                      >
                        ...
                        {/* <option value="đã bàn giao cho đơn vị vận chuyển">
