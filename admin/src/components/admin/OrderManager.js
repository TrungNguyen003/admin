...
const AdminOrders = ({
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:10000/admin/orders`, {
        params: {
          page,
          status: filterStatus,
  ...
  const deleteOrder = async (orderId) => {
    try {
      await axios.delete(`http://localhost:10000/admin/orders/${orderId}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        },
      });
            ...
            {orderDetails && (
              <>
                ...
                <div className="invoice-info">
                  ...
                  <p>
                    <strong>Trạng thái thanh toán:</strong> {orderDetails.paymentStatus}
                  </p>
                  <p>
                    <strong>Ngày:</strong>{" "}
