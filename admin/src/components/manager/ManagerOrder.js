...
const ManagerRoute = ({
  ...
  const fetchOrders = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:10000/manager/orders`,
        {
          params: {
            page,
            status: filterStatus,
  ...
  const fetchOrderDetails = async (orderId) => {
    try {
      const response = await axios.get(
        `http://localhost:10000/manager/orders/${orderId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
          },
  ...
  const updatePaymentStatus = async (orderId) => {
    try {
      ...
      await axios.put(
        `http://localhost:10000/manager/orders/${orderId}/payment`,
        updateData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
      ...
      // Cập nhật lại danh sách đơn hàng hoặc trạng thái đơn hàng
      fetchOrders();
      setStatus("");
      setRefundReason(""); 
      setIsUpdated(true);
  ...
  const deleteOrder = async (orderId) => {
    try {
      await axios.delete(`http://localhost:10000/manager/orders/${orderId}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        },
      });
  ...
  const handleCreateShipment = (order) => {
    navigate("/create-shipmentM", { state: { order } });
  };
  ...
  return (
    <div className="main-container-124">
      ...
      <div className="container-124">
                ...
                <tr key={order._id}>
                  ...
                  <td>
                    {new Intl.NumberFormat("vi-VN", {
                      style: "currency",
                      currency: "VND",
                    }).format(order.total + order.shippingFee)}
                  </td>
                  <td>{order.status}</td>
                  <td>
                    <Button
        ...
        {/* Modal hiển thị chi tiết đơn hàng giống một hóa đơn hoàn chỉnh */}
          ...
          <Modal.Body>
            {orderDetails && (
              <>
                ...
                {orderDetails.paymentStatus !== "đã nhận thanh toán" && (
                  <div>
                    <Form.Group>
                      ...
                      >
                        ...
                        <option value="đã nhận thanh toán">
                          Đã nhận thanh toán
                        </option>
